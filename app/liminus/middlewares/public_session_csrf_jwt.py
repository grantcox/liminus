...
class PublicSessionMiddleware(GkRequestMiddleware, CsrfHandlerMixin, JwtHandlerMixin):
    ...
    SESSION_ID_COOKIE_NAME = settings.PUBLIC_SESSION_COOKIE_NAME
    SESSION_COOKIE_DOMAIN = settings.PUBLIC_COOKIES_DOMAIN
    SESSION_IDLE_TIMEOUT_SECONDS = settings.PUBLIC_SESSION_IDLE_TIMEOUT_SECONDS
    SESSION_STRICT_MAX_LIFETIME_SECONDS = settings.PUBLIC_SESSION_STRICT_MAX_LIFETIME_SECONDS

    CSRF_HEADER_NAME = settings.PUBLIC_CSRF_HEADER_NAME
    ...
    JWKS_URL = settings.MEMBER_AUTH_JWKS_URL
    REFRESH_JWT_URL = settings.MEMBER_AUTH_JWT_REFRESH_URL
    REFRESH_JWT_IF_TTL_LESS_THAN_SECONDS = settings.MEMBER_AUTH_JWT_REFRESH_IF_TTL_LESS_THAN_SECONDS
    ...
    async def handle_request(self, req: Request, reqset: ReqSettings, backend: Backend):
        ...
        await self._verify_csrf_if_needed(req, session.session_id, reqset)
